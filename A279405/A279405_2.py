...
# (the maximal peaceable queen armies on a torus)
    ...
    for i in range(n):
        ...
        for k in range(n):
            ...
            if k != i:
                ...
                exec("m.addLConstr(" + s_x + " + " + p_y + "<= 1)")
            if k != j or (i-j+k)%n != i: 
                s_x = "x_" + str(k) + "_" + str((i-j+k)%n) 
                s_y = "y_" + str(k) + "_" + str((i-j+k)%n)
                exec("m.addLConstr(" + p_x + " + " + s_y + "<= 1)")
                exec("m.addLConstr(" + s_x + " + " + p_y + "<= 1)")
            if k != j or (2*i-(i-j+k))%n != i:
                s_x = "x_" + str(k) + "_" + str((2*i-(i-j+k))%n) 
                s_y = "y_" + str(k) + "_" + str((2*i-(i-j+k))%n)
                exec("m.addLConstr(" + p_x + " + " + s_y + "<= 1)")
                exec("m.addLConstr(" + s_x + " + " + p_y + "<= 1)")
